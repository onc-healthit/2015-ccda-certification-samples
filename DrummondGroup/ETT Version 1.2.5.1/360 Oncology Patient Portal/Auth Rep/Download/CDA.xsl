...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="n1:ClinicalDocument">
        <html>
            ...
            <body class="clinical_record_data">
                <h1 class="h1center">
                    <xsl:value-of select="$title"/>
                </h1>
                            ...
                            <xsl:choose>
                                <xsl:when test="n1:code">
                                    <tr>
                                        <td class="td_header_role_name">
                                            <span class="td_label">
                                                <xsl:text>Encounter Id / Type</xsl:text>
                                            </span>
                                        </td>
                                        <td class="td_header_role_value">
                                            <xsl:call-template name="show-id">
    ...
    <xsl:template name="recordTarget">
        <table id="summary" class="header_table">
                                 ...
                                 <xsl:when test="n1:patient/n1:languageCommunication/n1:languageCode">
                                     <xsl:for-each select="n1:patient/n1:languageCommunication/n1:languageCode">
                                     	<xsl:call-template name="show-language">
					                        <xsl:with-param name="langcode" select="@code"/>
					                    </xsl:call-template>
    ...
    <xsl:template name="section-title">
        ...
        <xsl:choose>
            <xsl:when test="count(/n1:ClinicalDocument/n1:component/n1:structuredBody/n1:component[n1:section]) &gt; 1">
                <h3>
                    <a id="{generate-id($title)}" href="#toc">
                        <xsl:value-of select="$title"/>
                    </a>
                </h3>
            </xsl:when>
        ...
        <xsl:choose>
            <xsl:when test="$asgnEntity/n1:assignedPerson/n1:name">
                ...
                <xsl:if test="$asgnEntity/n1:representedOrganization/n1:name">
                    ...
                    <xsl:for-each select="$asgnEntity/n1:representedOrganization/n1:addr/n1:streetAddressLine">
						<xsl:text>, </xsl:text>
						<xsl:value-of select="."/>
                    </xsl:for-each>
                    <xsl:text>, </xsl:text>
                    ...
                    <xsl:if test="$asgnEntity/n1:representedOrganization/n1:telecom">
	                    <xsl:text>. </xsl:text>
	                    <xsl:call-template name="show-telecom">
    				        <xsl:with-param name="telecom" select="$asgnEntity/n1:representedOrganization/n1:telecom"/>
				        </xsl:call-template>
                    </xsl:if>
                </xsl:if>
            </xsl:when>
...
.clinical_record_data {
  ...
  font-family: RobotoDraft, Roboto, 'Helvetica Neue', Segoe UI, Helvetica, Tahoma, sans-serif;
  font-size: 14px;
}

.clinical_record_data a, a:visited {
    text-decoration: none;
    color: #309CD7;
    text-transform: uppercase;
}

.clinical_record_data ul {
	list-style: none;
	padding: 0;
}

.clinical_record_data li>table {
}

.clinical_record_data hr {
	background-color: #ccc;
	border-color: #ccc;
    margin: 35px -8px 0 -8px;
}

.clinical_record_data h1 {
  font-size: 12pt;
  font-weight: bold;
}

.clinical_record_data h2 {
  font-size: 11pt;
  font-weight: bold;
}

.clinical_record_data h3 {
  font-size: 10pt;
  font-weight: bold;
}

.clinical_record_data h4 {
  font-size: 8pt;
  font-weight: bold;
}

.clinical_record_data table {
  width: 100%;
  font-size: 14px;
  border-spacing: 0;
...
.clinical_record_data table:empty {
	display: none;
}

.clinical_record_data th {
...
.clinical_record_data td[colspan="4"]:empty {
	border: none;
	border-bottom: none;
}

.clinical_record_data td[colspan="4"] {
	font-weight: bold;
	border: none;
	border-bottom: 1px solid #eaeaea;
}

.clinical_record_data td:not:first-child {
}

.clinical_record_data td {
  padding: 0.1cm 0.2cm;
  vertical-align: top;
...
.clinical_record_data tr:last-child td {
	border: none;
}

.clinical_record_data td:last-child {
	border-right: none;
}

.clinical_record_data .h1center {
  font-size: 14pt;
  font-weight: normal;
  text-align: center;
  width: 100%;
...
.clinical_record_data .header_table{
  border: none;
}

.clinical_record_data .td_header_role_name{
  text-align: right;
  color: black;
  width: 20%;
  font-weight: bold;
...
.clinical_record_data .td_header_role_value{
  width: 80%;
  border: none;
}

.clinical_record_data .Bold{
}

.clinical_record_data .Italics{
  font-style: italic;
}

.clinical_record_data .Underline{
